...
export default function ProfessionalDashboard() {
  ...
  React.useEffect(() => {
    if (mounted) {
      document.title = language === 'tr' ? 'Erashu Engine - Kurumsal Arbitraj' : 'Erashu Engine - Institutional Arbitrage';
    }
  }, [language, mounted]);

  if (!mounted || status === 'loading') return null; // Avoid hydration mismatch
    ...
    <div className="min-h-screen bg-[#080b10] text-slate-100 font-sans selection:bg-indigo-500/30">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            {session ? (
              ...
              <Link 
                href="/login"
                className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-600/20 mr-2"
              >
                {language === 'tr' ? 'GİRİŞ YAP' : 'SIGN IN'}
              </Link>
            )}
            <div className="flex items-center gap-3 pl-4 border-l border-slate-800">
              <div className="text-right hidden sm:block">
                <div className="text-[10px] font-black text-white leading-none mb-1">{session?.user?.name || (language === 'tr' ? 'Misafir' : 'Guest')}</div>
              ...
              <div className="relative group">
                <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-indigo-600 to-violet-600 border border-indigo-400/20 flex items-center justify-center text-[10px] font-black text-white shadow-lg shadow-indigo-500/20">
                  {session?.user?.name?.[0] || (language === 'tr' ? 'M' : 'G')}
                </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          ...
          <div className="lg:col-span-1">
            {/* System Status Summary (Legacy Integration) */}
